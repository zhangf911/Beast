...
/** A parser for decoding HTTP/1 wire format messages.
    ...
    such as @ref beast::http::read, @ref beast::http::async_read,
    @ref beast::http::parse, @ref beast::http::parse_some,
    @ref beast::http::async_parse, and
    @ref beast::http::async_parse_some. Alternatively, the caller
    may use custom techniques to ensure that the structured portion
    of the HTTP message (header or chunk header) is contained in a
    linear buffer.
